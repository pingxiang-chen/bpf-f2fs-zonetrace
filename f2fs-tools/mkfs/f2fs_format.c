...
static int f2fs_prepare_super_block(void)
{
	...
	MSG(0, "Info: need at least %"PRIu32" zones for metadata\n", 
			get_sb(main_blkaddr) / (c.segs_per_zone * c.blks_per_seg));
	...
	MSG(0, "Info: main_blkaddr: %"PRIu32"\n", get_sb(main_blkaddr));

	if (c.zoned_mode) {
		for (i = 0; i < c.ndevs; i++) {
			if(c.devices[i].zoned_model == F2FS_ZONED_HM){
				MSG(0, "Info: zoned devices %s\n", c.devices[i].path);
				MSG(0, "      start_blkaddr: %"PRIu64"\n", c.devices[i].start_blkaddr);
				MSG(0, "      end_blkaddr: %"PRIu64"\n", c.devices[i].end_blkaddr);
				MSG(0, "      zone_blocks: %zu\n", c.devices[i].zone_blocks);
				
#ifdef HAVE_BLK_ZONE_REP_V2
				for (j = 0; j < c.devices[i].nr_zones; j++) {
					MSG(0, "zone %d\n", j);
					MSG(0, "      zone_cap_blocks: %zu\n", c.devices[i].zone_cap_blocks[j]);
				}
#endif
			}
		}	
	...
	if (c.overprovision == 0)
		...
		c.default_ovp = false;
#ifdef HAVE_BLK_ZONE_REP_V2
		c.reserved_segments = c.overprovision * c.segs_per_sec;
		goto finish_set_gc_reserved;
#endif
	}
...
#ifdef HAVE_BLK_ZONE_REP_V2
	if (!c.default_ovp){
		MSG(0, "Info: Overprovision %d sections\n", (int)c.overprovision);
		goto print_msg;
	}
#endif
